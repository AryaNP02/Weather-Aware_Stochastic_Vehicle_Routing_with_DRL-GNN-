...
class InferenceStrategy:
    ...
    def solve(self, env: RoutingSimulator, initial_state=None, is_deterministic=False, **kwargs):
        raise NotImplementedError

class GreedyInference(InferenceStrategy):
    def solve(self, env: RoutingSimulator, initial_state=None, is_deterministic=False):
...
class RandomSamplingInference(InferenceStrategy):
    def solve(self, env: RoutingSimulator, num_samples=16, initial_state=None, is_deterministic=False):
        best_cost = float('inf')
        best_routes = None
...
class BeamSearchInference(InferenceStrategy):
    def solve(self, env: RoutingSimulator, beam_width=3, initial_state=None, is_deterministic=False):
        if initial_state is not None:
            env.set_clonable_state(initial_state)
            if is_deterministic:
                env.real_demands = env.base_demands.clone()
